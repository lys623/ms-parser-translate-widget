	...
	if (result){
			...
			if (dirExists){
				...
				files.forEach(function(item){
					...
					var cssCompileFn = function(staticUrl){
						//try {
						//	var data = fis.util.read(jdf.currentDir + staticUrl,true);
                        //
						//}catch (e){
						//	var data=fis.util.read(jdf.currentDir+staticUrl.replace('.css','.less'),true);
						//}
						//try{
						//	var file=fis.file(jdf.currentDir + staticUrl);
						//}catch (e){
						//	var file=fis.file(jdf.currentDir + (staticUrl.replace('.css','.less')));
						//}
						//console.log('file:'+file.getUrl())
						////console.log(data)
						//var md5=fis.util.md5(data);
						//console.log("_"+md5)
						...
						if (isBuild){
							...
							if(jdf.config.output.combineWidgetCss){
								...
								if(jdf.config.output.cssCombo && settings.cdn){
					...
					var jsCompileFn = function(staticUrl){
						...
						if (isBuild){
							...
							if(jdf.config.output.combineWidgetJs){
								...
								if(jdf.config.output.jsCombo && settings.cdn){
					...
					if (($.is.css(item)||$.is.less(item)||$.is.sass(item) )&& !filesListObj[staticUrl] && buildTag.css ){
					...
					if (($.is.js(item)||$.is.es(item) )&& !filesListObj[staticUrl] && buildTag.js){
		...
		if (isRelease || isOutput){
			...
			if(jdf.config.output.combineWidgetCss && cssFile !=''){
				...
				outputCss1 = settings.cdn + $.replaceSlash(outputCss1);
			...
			if(jdf.config.output.combineWidgetJs && jsFile !=''){
				...
				outputJs1 = settings.cdn + $.replaceSlash(outputJs1);
	...
	var replaceCore= function (str,type){
		...
		if (regResult){
			...
			regResult.forEach(function(item){
				var reg = new RegExp(regStr,'gm');
				var i = reg.exec(item);
				var cdnRegStr = settings.cdnDefalut ? settings.cdnDefalut : settings.cdn;
			...
			if(comboArray.length>0){
				...
				//combo
				if(jdf.config.output[type+'Combo'] && settings.cdn){
					var cdnPrefix = '';
					cdnPrefix =  settings.cdn + (comboArray.length>1 ? '/??' : '/');
					...
					for (var i=0; i<comboArray.length; i++){
						...
						item = settings.cdn ? settings.cdn+'/'+item : item;
