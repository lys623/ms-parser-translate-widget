...
var newSmarty = new Smarty();
newSmarty.conf.left_delimiter="{{";
newSmarty.conf.right_delimiter="}}";
...
vm.rander = function(vmSource, dataObj, dirname){
	//var newSmarty ={render:function(){}}
	if (vmSource && dataObj) {
		var vmTpm = vm.parse&&vm.parse(vmSource, dirname);
		//content:Velocity.render(vmTpm.content, dataObj),
		//console.log('111:'+newSmarty.render)
		//var content=newSmarty.render&&newSmarty.render(vmTpm.content,dataObj);
		var content=newSmarty.render(vmTpm.content,dataObj);

		return {
			//content:newSmarty.render(vmTpm.content,dataObj),
			content:content,
			url:vmTpm.url
		};
	}
